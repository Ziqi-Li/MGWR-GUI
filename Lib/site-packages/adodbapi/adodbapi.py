...
if api.onIronPython:
    ...
    try:
        ...
        import warnings
        warnings.warn("pywin32 package (or IronPython) required for adodbapi.",
                      ImportWarning)
...
def make_COM_connecter():
    try:
        ...
        c = Dispatch(
            'ADODB.Connection')  #connect _after_ CoIninialize v2.1.1 adamvan
    except:
...
class Connection(object):
    ...
    def connect(self, kwargs, connection_maker=make_COM_connecter):
        ...
        self.dbms_name = getIndexedValue(self.connector.Properties,
                                         'DBMS Name').Value
        self.dbms_version = getIndexedValue(self.connector.Properties,
                                            'DBMS Version').Value
        ...
        if 'paramstyle' in kwargs:
            self.paramstyle = kwargs[
                'paramstyle']  # let setattr do the error checking
        self.messages = []
        if verbose:
    ...
    def __setattr__(self, name, value):
        if name == 'autocommit':  # extension: allow user to turn autocommit on or off
            ...
            if value not in api.accepted_paramstyles:
                self._raiseConnectionError(
                    api.NotSupportedError, 'paramstyle="%s" not in:%s' %
                    (value, repr(api.accepted_paramstyles)))
        elif name == 'variantConversions':
            value = copy.copy(
                value)  # make a new copy -- no changes in the default, please
    ...
    def printADOerrors(self):
        ...
        for e in self.connector.Errors:
            ...
            print('Error: %s %s ' % (e.Number,
                                     adc.adoErrors.get(e.Number, "unknown")))
...
class Cursor(object):
    ...
    def __init__(self, connection):
        ...
        if verbose:
            print('%s New cursor at %X on conn %X' % (version, id(self),
                                                      id(self.connection)))
        ...
        for i in range(self.numberOfColumns):
            ...
            try:
                self.converters.append(
                    varCon[f.Type])  # conversion function for this column
            except KeyError:
                self._raiseCursorError(
                    api.InternalError,
                    'Data column of Unknown ADO type=%s' % f.Type)
    ...
    def close(self, dont_tell_me=False):
        ...
        if not dont_tell_me:
            self.connection._i_am_closing(
                self)  # take me off the connection's cursors list
        self.messages = []
    ...
    def callproc(self, procname, parameters=None):
        ...
        if verbose > 2:
            print('Calling Stored Proc with Params=',
                  format_parameters(self.cmd.Parameters, True))
    ...
    def execute(self, operation, parameters=None):
        ...
        if self.command is not operation or self._ado_prepared == 'setup':
            ...
            if parameters and self.paramstyle != 'qmark':
                operation = self._reformat_operation(
                    operation,
    ...
    def nextset(self):
        ...
        if api.onIronPython:
            ...
            try:  #[begin 2.1 ekelund]
                ...
                self._raiseCursorError(api.NotSupportedError,
    ...
    def _last_query(
            self):  # let the programmer see what query we actually used
        try:
            if self.parameters == None:
                ret = self.cmd.CommandText
            else:
                ret = "%s,parameters=%s" % (self.cmd.CommandText,
