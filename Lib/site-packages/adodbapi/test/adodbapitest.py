...
class CommonDBTests(unittest.TestCase):
    ...
    def testDefaultErrorHandlerConnection(self):
        if not self.remote:
            ...
            try:
                conn.close()
                conn.commit(
                )  #Should not be able to use connection after it is closed
            except:
    ...
    def testOwnErrorHandlerConnection(self):
        ...
        assert len(
            conn.messages
        ) > 0, "Setting errorhandler to none  should bring back the standard error handler"
    ...
    def testUserDefinedConversions(self):
        ...
        try:
            ...
            if not self.remote:
                ...
                crsr.execute("SELECT fldData,fld2 FROM xx_%s ORDER BY fldData"
                             % config.tmp)
                ...
                self.assertEquals(row[1],
                                  'YO')  # second will convert to upper case
    ...
    def testUserDefinedConversionForExactNumericTypes(self):
        ...
        if not self.remote and sys.version_info < (
            ...
            try:
                ...
                adodbapi.variantConversions[
                    ado_consts.
                    adNumeric] = lambda x: '!!This function returns a funny unicode string %s!!' % x
                self.helpTestDataType(
                    "numeric(18,2)",
                    'NUMBER',
                    '3.45',
                ...
                adodbapi.variantConversions[
                    ado_consts.
                    adNumeric] = oldconverter  #Restore the original convertion function

    def helpTestDataType(self,
                         sqlDataTypeString,
                         DBAPIDataTypeString,
                         pyData,
        ...
        assert descTuple[0] in [
            'fldData', 'flddata'
        ], 'was "%s" expected "%s"' % (descTuple[0], 'fldData')
        ...
        for inParam in inputs:
            ...
            try:
                ...
                raise
            crsr.execute(
                "SELECT fldData FROM xx_%s WHERE ?=fldID" % config.tmp,
                [fldId])
            rs = crsr.fetchone()
            if allowedReturnValues:
                allowedTypes = tuple(
                    [type(aRV) for aRV in allowedReturnValues])
            ...
            if compareAlmostEqual and DBAPIDataTypeString == 'DATETIME':
                ...
                if allowedReturnValues:
                    ok = False
                    self.assertTrue(
                        rs[0] in allowedReturnValues, 'Value "%s" not in %s' %
                        (repr(rs[0]), allowedReturnValues))
                else:
                    self.assertEquals(
                        rs[0], pyData,
    ...
    def testDataTypeFloat(self):
        ...
        self.helpTestDataType(
            "float", 'NUMBER', 1.79e37, compareAlmostEqual=True)
    ...
    def testDataTypeDecmal(self):
        ...
        self.helpTestDataType(
            ...
            444444444444444444,
            allowedReturnValues=[
                '444444444444444444.00', '444444444444444444,00',
    ...
    def testDataTypeMoney(self):  #v2.1 Cole -- use decimal for money
        if self.getEngine() == 'MySQL':
            ...
            self.helpTestDataType("money", 'NUMBER',
                                  decimal.Decimal('-922337203685477.5808'))
    ...
    def testDataTypeInt(self):
        ...
        if self.getEngine() not in ['ACCESS', 'PostgreSQL']:
            self.helpTestDataType("bit", 'NUMBER',
                                  1)  #Does not work correctly with access
    ...
    def testDataTypeVarChar(self):
        if self.getEngine() == 'MySQL':
            ...
            stringKinds = ["varchar(10)", "nvarchar(10)", "text",
                           "ntext"]  #,"varchar(max)"]
    ...
    def helpCreateAndPopulateTableTemp(self, crsr):
        ...
        for i in range(9):  # note: this poor SQL code, but a valid test
            crsr.execute(
                "INSERT INTO xx_%s (fldData) VALUES (%i)" % (config.tmp, i))
    ...
    def testExecuteMany(self):
        ...
        crsr.executemany("INSERT INTO xx_%s (fldData) VALUES (?)" % config.tmp,
                         seq_of_values)
        if crsr.rowcount == -1:
    ...
    def testRowIterator(self):
        ...
        for inParam in inputs:
            ...
            for j in range(len(inParam)):
                assert rec[j] == inParam[
                    j], 'returned value:"%s" != test value:"%s"' % (rec[j],
                                                                    inParam[j])
            # check that we can get a complete tuple from a row
            assert tuple(
                rec) == inParam, 'returned value:"%s" != test value:"%s"' % (
    ...
    def testFormatParamstyle(self):
        ...
        for inParam in inputs:
            ...
            try:
                ...
                raise
            crsr.execute(
                "SELECT fldData, fldConst FROM xx_" + config.tmp +
                " WHERE %s=fldID", [fldId])
            rec = crsr.fetchone()
            self.assertEqual(
                rec[0], inParam,
    ...
    def testNamedParamstyle(self):
        ...
        for inParam in inputs:
            ...
            try:
                ...
                raise
            crsr.execute(
                "SELECT fldData FROM xx_%s WHERE :Id=fldID" % config.tmp,
                {'Id': fldId})
            rec = crsr.fetchone()
            self.assertEqual(
                rec[0], inParam,
    ...
    def testPyformatParamstyle(self):
        ...
        for inParam in inputs:
            ...
            try:
                ...
                raise
            crsr.execute(
                "SELECT fldData FROM xx_%s WHERE %%(Id)s=fldID" % config.tmp,
                {'Id': fldId})
            rec = crsr.fetchone()
            self.assertEqual(
                rec[0], inParam,
    ...
    def testRollBack(self):
        ...
        self.conn.rollback()
        crsr.execute(selectSql)
        assert crsr.fetchone(
        ) == None, 'cursor.fetchone should return None if a query retrieves no rows'
        crsr.execute('SELECT fldData from xx_%s' % config.tmp)
        rs = crsr.fetchall()
...
class TestADOwithSQLServer(CommonDBTests):
    def setUp(self):
        self.conn = config.dbSqlServerconnect(*config.connStrSQLServer[0],
    ...
    def testVariableReturningStoredProcedure(self):
        ...
        assert retvalues[
            2] == 'DodsworthAnne', '%s is not "DodsworthAnne"' % repr(
                retvalues[2])
        self.conn.rollback()
    ...
    def testMultipleSetReturn(self):
        ...
        assert crsr.nextset(
        ) == None, 'No more return sets, should return None'
    ...
    def testIncorrectStoredProcedureParameter(self):
        ...
        result = tryconnection.try_operation_with_expected_exception(
            (api.DataError, api.DatabaseError), crsr.callproc,
            ['sp_DeleteMeOnlyForTesting'],
...
class TestADOwithAccessDB(CommonDBTests):
    def setUp(self):
        self.conn = config.dbAccessconnect(*config.connStrAccess[0],
                                           **config.connStrAccess[1])
        self.conn.timeout = 30  # turn timeout back up
        self.engine = 'ACCESS'
...
class TestADOwithMySql(CommonDBTests):
    def setUp(self):
        self.conn = config.dbMySqlconnect(*config.connStrMySql[0],
                                          **config.connStrMySql[1])
        self.conn.timeout = 30  # turn timeout back up
        self.engine = 'MySQL'
...
class TestADOwithPostgres(CommonDBTests):
    def setUp(self):
        self.conn = config.dbPostgresConnect(*config.connStrPostgres[0],
...
class TestPythonTimeConverter(TimeConverterInterfaceTest):
    ...
    def testCOMDate(self):
        mk = time.mktime((2002, 6, 28, 18, 15, 1, 4,
                          31 + 28 + 31 + 30 + 31 + 28, -1))
        t = time.localtime(mk)
        # Fri, 28 Jun 2002 18:15:01 +0000
        cmd = self.tc.COMDate(t)
        assert abs(
            cmd - 37435.7604282) < 1.0 / 24, "%f more than an hour wrong" % cmd

    def testDateObjectFromCOMDate(self):
        cmd = self.tc.DateObjectFromCOMDate(37435.7604282)
        t1 = time.gmtime(
            time.mktime((2002, 6, 28, 0, 14, 1, 4, 31 + 28 + 31 + 30 + 31 + 28,
                         -1)))
        #there are errors in the implementation of gmtime which we ignore
        t2 = time.gmtime(
            time.mktime((2002, 6, 29, 12, 14, 2, 4,
                         31 + 28 + 31 + 30 + 31 + 28, -1)))
        assert t1 < cmd < t2, '"%s" should be about 2002-6-28 12:15:01' % repr(
            cmd)

    def testDate(self):
        t1 = time.mktime((2002, 6, 28, 18, 15, 1, 4,
                          31 + 28 + 31 + 30 + 31 + 30, 0))
        t2 = time.mktime((2002, 6, 30, 18, 15, 1, 4,
                          31 + 28 + 31 + 30 + 31 + 28, 0))
    ...
    def testTimestamp(self):
        t1 = time.localtime(
            time.mktime((2002, 6, 28, 18, 14, 1, 4,
                         31 + 28 + 31 + 30 + 31 + 28, -1)))
        t2 = time.localtime(
            time.mktime((2002, 6, 28, 18, 16, 1, 4,
                         31 + 28 + 31 + 30 + 31 + 28, -1)))
...
class TestPythonDateTimeConverter(TimeConverterInterfaceTest):
    ...
    def testDateObjectFromCOMDate(self):
        ...
        tx = datetime.datetime(
            2002, 6, 28, 18, 14, 1,
            900000)  # testing that microseconds don't become milliseconds
...
if __name__ == '__main__':
    with cleanup_manager():
        ...
        if config.iterateOverTimeTests:
            for test, dateconverter, tag in ((config.doTimeTest,
                                              api.pythonTimeConverter,
                                              'pythontime'),
                                             (config.doMxDateTimeTest,
                                              api.mxDateTimeConverter, 'mx')):
                if test:
