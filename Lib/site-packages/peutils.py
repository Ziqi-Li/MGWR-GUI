...
class SignatureDatabase(object):
    ...
    def generate_section_signatures(self, pe, name, sig_length=512):
        ...
        for idx, section in enumerate(pe.sections):
            ...
            sig_name = '%s Section(%d/%d,%s)' % (name, idx + 1, len(
                pe.sections), ''.join(
                    [c for c in section.Name if c in string.printable]))
    ...
    def __match(self, pe, ep_only, section_start_only):
        ...
        if section_start_only is True:
            ...
            #
            scan_addresses = [
                section.PointerToRawData for section in pe.sections
            ]
    ...
    def __match_signature_tree(self, signature_tree, data, depth=0):
        ...
        #
        for idx, byte in enumerate(
            [b if isinstance(b, int) else ord(b) for b in data]):
            ...
            if '??' in match:
                ...
                if data_remaining:
                    matched_names.extend(
                        self.__match_signature_tree(match_tree_alternate,
                                                    data_remaining,
                                                    idx + depth + 1))
