...
class Sel_BW(object):
    ...
    def __init__(self, coords, y, X_loc, X_glob=None, family=Gaussian(),
                 ...
                 constant=True, spherical=False,n_jobs=-1,ext_w=None,raw_y=None,raw_x=None):
    ...
    def _bw(self):
        gwr_func = lambda bw: getDiag[self.criterion](GWR(
            ...
            offset, spherical=self.spherical,ext_w=self.ext_w,n_jobs=self.n_jobs).fit(lite=True))
    ...
    def _mbw(self):
        ...
        def gwr_func(y, X, bw,family=Gaussian(),offset=None,ext_w=None):
            return GWR(coords, y, X, bw, family=family, kernel=kernel,
                       ...
                       spherical=self.spherical, hat_matrix=False,n_jobs=self.n_jobs,ext_w=ext_w).fit(
                           lite=True)
        ...
        def bw_func(y, X,family=Gaussian(),offset=None,ext_w=None):
            selector = Sel_BW(coords, y, X, X_glob=[], family=family,
                              ...
                              constant=False, spherical=self.spherical,ext_w=ext_w,n_jobs=self.n_jobs)
            return selector
