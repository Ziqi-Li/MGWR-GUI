...
class Sel_BW(object):
    ...
    def __init__(self, coords, y, X_loc, X_glob=None, family=Gaussian(),
                 ...
                 constant=True, spherical=False,ext_w=None,raw_y=None,raw_x=None):
    ...
    def search(self, search_method='golden_section', criterion='AICc',
               ...
               multi_bw_max=[None], bws_same_times=5, pool=None, verbose=False):
        """
    ...
    def _bw(self):
        gwr_func = lambda bw: [getDiag[self.criterion](GWR(
            ...
            offset, spherical=self.spherical,ext_w=self.ext_w,).fit(lite=True, pool=self.pool))]
    ...
    def _mbw(self):
        ...
        def gwr_func(y, X, bw,family=Gaussian(),offset=None,ext_w=None):
            return GWR(coords, y, X, bw, family=family, kernel=kernel,
                       fixed=fixed, offset=offset, constant=False,
                       spherical=self.spherical, hat_matrix=False,ext_w=ext_w).fit(
                           lite=True, pool=self.pool)

        def bw_func(y, X,family=Gaussian(),offset=None,ext_w=None):
            selector = Sel_BW(coords, y, X, X_glob=[], family=family,
                              kernel=kernel, fixed=fixed, offset=offset,
                              constant=False, spherical=self.spherical,ext_w=ext_w)
            return selector
        ...
        self.bw = multi_bw(self.init_multi, y, X, n, k, family, self.tol_multi,
                           ...
                           bws_same_times, verbose=self.verbose,raw_y=self.raw_y,
                           raw_x=self.raw_x,ext_w=self.ext_w,offset=self.offset)
