...
class GWRResults(GLMResults):
    ...
    def get_bws_intervals(self, selector, pval=0.95):
        ...
        aiccs = pd.DataFrame([i[0] for i in list(zip(*selector.sel_hist))[1]],columns=["aicc"])
...
class MGWRResults(GWRResults):
    ...
    def get_bws_intervals(self, selector, pval=0.95):
        ...
        for j in range(self.k):
            #Get AICcs and associated bw from the last iteration of back-fitting and make a DataFrame
            aiccs = pd.DataFrame([i[0] for i in list(zip(*selector.sel_hist[-self.k+j]))[1]],columns=["aicc"])
            aiccs['bw'] = list(zip(*selector.sel_hist[-self.k]))[0]
            #Sort DataFrame by the AICc values
            aiccs = aiccs.sort_values(by=['aicc'])
            #Calculate delta AICc
            d_aic_ak = aiccs.aicc - aiccs.aicc.min()
